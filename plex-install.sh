...
TESTINGBRANCH="https://github.com/JRGTH/nas4free-plex-extension/archive/testing.zip"
MASTERBRANCH="https://github.com/JRGTH/nas4free-plex-extension/archive/master.zip"
...
fetch_branch()
{
	...
	if [ "${BRANCH}" = "testing" ]; then
		...
		/usr/bin/fetch -avo ${CWDIR} --no-verify-peer --timeout=30 ${TESTINGBRANCH} || error_notify "Error: A problem has occurred while fetching testing branch."
		/bin/mv ${CWDIR}/testing.zip ${CWDIR}/master.zip
	else
		# Fetch the latest master branch.
		echo "=> Retrieving the latest master branch..."
		/usr/bin/fetch -avo ${CWDIR} --no-verify-peer --timeout=30 ${MASTERBRANCH} || error_notify "Error: A problem has occurred while fetching master branch."
	fi
}
...
install_main()
{
	...
	/bin/rm -f "${CWDIR}/master.zip" "${CWDIR}/README.md" "${CWDIR}/plex-install.php" "${CWDIR}/plex-install.sh"
